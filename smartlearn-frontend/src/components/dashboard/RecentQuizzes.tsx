...
import { BarChart2, Clock, Play } from 'lucide-react';
...
const RecentQuizzes: React.FC<RecentQuizzesProps> = ({ quizzes }) => {
  ...
  return (
    <div className="bg-bg-secondary rounded-xl p-6 border border-border-primary shadow-sm h-full">
      ...
      <div className="space-y-4">
        ...
        {quizzes.length > 0 && (
          ...
          >
            <button className="w-full text-center text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-medium text-sm py-2 transition-colors">
              View All Quizzes →
            </button>
          </motion.div>
        )}
