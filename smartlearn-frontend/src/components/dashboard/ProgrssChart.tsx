        ...
        <div>
          ...
          <div className="flex items-end justify-between h-32 gap-2">
            {data.map((day, index) => (
              ...
              >
                <div
                  className="w-full bg-gradient-to-t from-primary-500 to-primary-300 rounded-t-lg transition-all duration-500 hover:opacity-80 cursor-pointer relative group"
                  style={{ height: `${(day.score / maxScore) * 80}%` }}
                >
                ...
                <span className="text-xs text-text-tertiary mt-2">{day.date}</span>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-6">
          ...
          <div className="flex items-end justify-between h-20 gap-2">
            {data.map((day, index) => (
              ...
              >
                <div
                  className="w-full bg-gradient-to-t from-success-500 to-success-300 rounded-t-lg transition-all duration-500 hover:opacity-80 cursor-pointer relative group"
                  style={{ height: `${(day.attempts / maxAttempts) * 60}%` }}
                >
                  <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-text-primary text-bg-primary text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">
